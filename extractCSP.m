...
%   filters selected are the one corresponding to the lowest and highest eigenvalues
%Output:
%features: the features extracted from this EEG data set as a [Nt * (nbFilterPairs*2 + 1)] matrix, with the class labels as the last column   
%initializations
nbTrials = size(EEGSignals.x_train,3);
features = zeros(nbTrials, 2*nbFilterPairs+1);
Filter = CSPMatrix([1:nbFilterPairs (end-nbFilterPairs+1):end],:);
%extracting the CSP features from each trial
for t=1:nbTrials    
    %projecting the data onto the CSP filters    
    projectedTrial = Filter * EEGSignals.x_train(:,:,t)';  
